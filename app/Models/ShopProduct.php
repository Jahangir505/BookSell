 ...
 */
    public static function getListSigle()
    {
        if (self::$listSingle == null) {
            self::$listSingle = self::get()->keyBy('id')->toArray();
        }
 ...
 */
    ...
    {
        return self::orderBy($orderBy, $sort)
 ...
 */
    ...
    public function ajaxSearch($keyword){
       ...
       $results = $this->where('status', 1)->with(['descriptions' => function ($q) use ($lang) {
                 $q->where('lang', $lang);
            }])
            ->with('promotionPrice')
 ...
 */
    ...
    // get seller product
    public static function getSellerProduct($seller_id)
    {
        $sellerProductsId = self::select('id')->where('user_id', $seller_id)->get();
        return $sellerProductsId;
    }

    public function scopeBestSales($query)
    {
        $query->whereNotIn('sold', ['0', 'null']);
